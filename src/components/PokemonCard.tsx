...
export const PokemonCard = React.memo(({ pokemon, index }: Props) => {
    return (
      <Card key={index} className={cn(`w-40 flex flex-col self-stretch drop-shadow hover:-translate-y-2 hover:drop-shadow-lg hover:shadow-md hover:shadow-${pokeTypeToTW(pokemon.types[0].typeName)} cursor-pointer transition-all ease-in-out duration-200`)}>
          ...
          <div className="flex justify-end items-center gap-1">
            {pokemon.types.map((type: any, index: number) => (
              pokeTypeToImg(type.typeName) !== "" && (
                <div key={index} className={cn(`bg-${pokeTypeToTW(type.typeName)} p-1.5 rounded-full`)}>
